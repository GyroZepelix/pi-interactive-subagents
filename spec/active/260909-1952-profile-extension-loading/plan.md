...
- **R07 - Child isolation and activation:** New Pi children launch with global extension discovery and initial built-ins disabled. Required framework controls load first, followed by canonical profile extension paths in resolved order. Only validated `builtin-tools` are activated before the first model request, while startup and later tools from declared extensions remain active.
- **R08 - Framework controls and nesting:** `ask_question` remains available to every named Pi child. `subagent_agents` remains the only nested-spawn grant; the spawning extension loads only when that list is non-empty and `PI_SUBAGENT_ALLOWED` remains pinned to the declared effective names. Profile extensions cannot replace framework controls through ordinary duplicate-name precedence.
- **R09 - Snapshot and resume:** New sidecars use an explicit version and capability mode and store selected built-ins plus the exact ordered absolute extension paths needed to replay the resolved loadout. Resume does not reread profiles or package settings, validates every stored path before pane creation, preserves framework-first ordering, and uses current installed contents at valid snapshotted paths.
...
Construct new Pi launches with `--no-extensions --no-builtin-tools`. Add the always-required child-control extension first, the nesting/spawning extension next only when granted, and then declared profile extensions in resolved order. Do not emit a strict `--tools` argument for the new capability mode.

Pass the validated built-in list through a private launch/resume value owned by this package. During `session_start`, the child-control extension selects those built-ins together with registered extension tools before the first model request. Preserve Pi's non-strict dynamic extension-tool activation behavior and deterministic first-loaded custom-tool precedence. Loading framework extensions first protects their tool names from ordinary profile-extension collisions.
...
- [ ] T02: Implement and verify the complete capability lifecycle
  ...
  - Expected areas: `pi-extension/subagents/index.ts`, `pi-extension/subagents/subagent-done.ts`, `pi-extension/subagents/session.ts`, `test/test.ts`
  - Acceptance: New launches and resumes replay the same framework-first extension grant and built-in subset; extension behavior follows the confirmed collision semantics; missing paths fail before pane creation; valid legacy strict snapshots still resume; removed interfaces have no remaining runtime or test references.
  - Verification: `node --test --test-name-pattern='session.ts|subagent discovery|subagent-done.ts|tool registration' test/test.ts`, `npm test`, and the controlled non-model tmux surface suite where available
...
- New commands contain `--no-extensions --no-builtin-tools`, omit strict `--tools`, load framework controls before profile extensions, and carry only validated built-ins to startup activation.
- Before the first model turn, selected built-ins and every startup extension tool are active, undeclared built-ins are inactive, dynamic extension tools become active, built-in overrides work, and first-loaded duplicate custom tools win.
...
- Extend `subagent-done.ts` tests with a mock ExtensionAPI/session-start fixture proving pre-model activation, extension-tool preservation, dynamic registration, overrides, and deterministic load order. Any small exported pure helper must remain package-internal and justified by this behavior seam.
...
- `node --test --test-name-pattern='session.ts|subagent discovery|subagent-done.ts|tool registration' test/test.ts`
- `npm test`
...
- **Activation timing:** Built-ins activated after startup could miss the first model turn or deactivate extension tools. Mitigate with a child-control `session_start` path and focused lifecycle tests.
